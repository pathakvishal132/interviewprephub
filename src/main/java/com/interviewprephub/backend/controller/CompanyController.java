...
public class CompanyController {
    ...
    public ResponseEntity<?> filterQuestions(
            ...
            @RequestParam(required = false) String searchText,
            @RequestParam(defaultValue = "1") int page) {
        Map<String, Object> result = companyService.getFilteredQuestion(level, role, minExperience, maxExperience,
                description, searchText, page);
    ...
    public ResponseEntity<?> getCompanyReviews(
            ...
            @RequestParam(defaultValue = "1") int page) {
        try {
            Map<String, Object> result = companyService.getCompanyReviews(company_id, page);
